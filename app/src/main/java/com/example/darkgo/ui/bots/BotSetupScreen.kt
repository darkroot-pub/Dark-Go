...
@Composable
fun BotSetupScreen(
    onStartBotMatch: (botCount: Int) -> Unit,
    onBack: () -> Unit
) {
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text("Vs Bots Setup", color = DarkGoTextPrimary, fontWeight = FontWeight.Bold) },
                navigationIcon = {
                    IconButton(onClick = onBack, modifier = Modifier.testTag("bot_setup_back_button")) {
        ...
        Column(
            ...
            Card(
                ...
                Column(
                    ...
                    Text(
                        text = "Play Against AI Bots",
                        style = MaterialTheme.typography.titleLarge.copy(fontWeight = FontWeight.Bold),
                        color = DarkGoTextPrimary
                    )
                    Spacer(modifier = Modifier.height(6.dp))
                    Text(
                        text = "Bots follow identical rules, hold valid 5×5 cards, take turns, and strategically complete horizontal rows without cheating.",
                    ...
                    FilterChip(
                        ...
                        label = {
                            Text(
                                ...
                                fontWeight = if (selectedBotCount == count) FontWeight.Bold else FontWeight.Normal
                            )
                        },
                        modifier = Modifier.weight(1f).testTag("bot_count_$count"),
                        colors = FilterChipDefaults.filterChipColors(
                            selectedContainerColor = DarkGoPrimary,
                            selectedLabelColor = DarkGoBackground,
                            containerColor = DarkGoSurfaceVariant,
                            labelColor = DarkGoTextPrimary
                        )
                    )
                }
            }
            ...
            Button(
                onClick = { onStartBotMatch(selectedBotCount) },
                modifier = Modifier
                    .fillMaxWidth()
                    .height(54.dp)
                    .testTag("start_bot_match_button"),
                colors = ButtonDefaults.buttonColors(containerColor = DarkGoPrimary),
                shape = RoundedCornerShape(12.dp)
            ) {
                Icon(Icons.Default.PlayArrow, contentDescription = "Continue", tint = DarkGoBackground)
                Spacer(modifier = Modifier.width(8.dp))
                Text(
                    text = "Continue to Card Creation",
